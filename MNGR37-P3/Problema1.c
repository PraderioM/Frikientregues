...
int main(){
	...
	while (n<65){
		...
		DibuixarGrafica(X,px,181, sprintf('Polinomio Interpolador con %d nodos equiespaiados',n));
		/*Calculamos los nodos equiespaiados*/
		N=NodesChebyschev(-1,1,n);
		/*calculamos la diferencia entre el polinomio interpolador y la función*/
		px=CalcularDiferencia(N, X, fx, n, 181);
		/*dibujamos la gràfica*/
		DibuixarGrafica(X,px,181, sprintf('Polinomio Interpolador con %d nodos de Chebyschev',n));
		n*=2;
	}
	return 0;
}
